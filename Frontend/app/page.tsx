...
export default function Home() {
  ...
  return <main className={dark ? "dark" : ""}>
    ...
    <section id="top" className="hero shell">
      <div className="hero-copy">
        ...
        <div className="order-box">
          <div className="tabs"><button className="active">Search medicine</button><button onClick={() => submit("Tell MediAssist your symptoms in any language.")}>Ask MediAssist <Sparkles size={14}/></button></div>
          <div className="search-row"><Search size={21}/><input value={query} onChange={e=>setQuery(e.target.value)} onKeyDown={e=>e.key === "Enter" && submit(`Searching nearby pharmacies for ${query || "medicine"}...`)} placeholder="Search by medicine or brand name"/><button onClick={()=>submit(`Searching nearby pharmacies for ${query || "medicine"}...`)}>Find medicine <ArrowRight size={17}/></button></div>
          {!query && <div className="suggestions">Popular: {medicines.map(m=><button key={m} onClick={()=>setQuery(m)}>{m}</button>)}</div>}
        </div>
      ...
      <div className="hero-visual" aria-label="Fast medicine delivery illustration">
    ...
    <section className="quick shell"><button onClick={()=>submit("Upload prescription flow opened.")}><span className="icon peach"><FileUp/></span><span><b>Upload prescription</b><small>We&apos;ll verify every detail</small></span><ArrowRight/></button><button onClick={()=>submit("Voice assistant activated.")}><span className="icon lavender"><Mic/></span><span><b>Speak to MediAssist</b><small>Any language, anytime</small></span><ArrowRight/></button><button onClick={()=>submit("Nearby pharmacy finder opened.")}><span className="icon mint"><MapPin/></span><span><b>Find nearby pharmacies</b><small>Medicines in your neighbourhood</small></span><ArrowRight/></button></section>
