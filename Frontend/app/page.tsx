    ...
    <main className={dark ? "dark" : ""}>
      ...
      <section id="top" className="hero shell">
        ...
        <div className="hero-visual" aria-label="Fast medicine delivery illustration" onClick={() => router.push("/medicines")} style={{ cursor: "pointer" }}>
          ...
          <div className="pharmacy-card"><div className="store-icon"><Store size={25}/></div><div><b>Nearest Verified Pharmacy</b><p><span className="dot"></span> 0.8 km away · Open now</p></div><div className="eta"><Clock3 size={16}/><b>8 min</b></div></div>
      ...
      <section id="partners" className="partner"><div className="shell partner-inner"><div><span className="kicker">FOR LOCAL PHARMACIES</span><h2>Your neighbourhood.<br/><em>Your customers.</em></h2><p>Join a delivery network that keeps local pharmacies at the heart of healthcare.</p><button onClick={() => router.push("/shopkeeper/dashboard")}>Partner with MediMall <ArrowRight size={17}/></button></div><div className="dashboard"><div className="dash-top"><span>Live Pharmacy Terminal</span><span className="status">● Online</span></div><div className="metric"><p>Live Dispatch Queue</p><h3>Prescription Review</h3><div className="patient"><span>RX</span><div><b>Incoming Prescription Order</b><small>Hyperlocal doorstep fulfillment</small></div><button onClick={() => router.push("/shopkeeper/dashboard")}>Review</button></div></div><div className="dash-bottom"><span>Today&apos;s orders <b>Live</b></span><span>Avg. dispatch <b>4 min</b></span></div></div></div></section>
